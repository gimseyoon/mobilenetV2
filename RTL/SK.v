...
module SK #(
    ...
    output reg                              result_save_valid,          // 1??? ???
    output reg                              skip_done                   // 64?????? ??? ???
);
    // -------------------------
    // ????/??????/?????
    // -------------------------
    ...
    reg  [PIXEL-1:0] wr_sel;               // one-hot write ??????
    wire             last_write = wr_sel[PIXEL-1];

    reg  [6:0] sk_cnt;                     // ?????? ??(0..63)
    reg        skip_done_pending;          // result_save_valid?? ???????
    ...
    wire signed [IO_WIDTH-1:0] sum_w   = in1_sel + in2_sel;   // 1??? ???(??? ???)
        ...
        if (!rst_n) begin
            ...
            // ?????? ????
                ...
                if (last_write) begin
                    busy              <= 1'b0;
                    wr_sel            <= {PIXEL{1'b0}};
                    save_pending      <= 1'b1;              // ???? ????? pack & valid
                    skip_done_pending <= (sk_cnt == 7'd63); // 64??¡Æ?????? ????
            ...
            if (save_pending) begin
                result_q          <= packed_result;     // 196?? ?? ???? ???
                result_save_valid <= 1'b1;              // 1??? ???
                skip_done         <= skip_done_pending; // 64??¡Æ ???? ???
                skip_done_pending <= 1'b0;
                save_pending      <= 1'b0;
            end
        end
